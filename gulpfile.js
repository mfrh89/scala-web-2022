...
const js = () => {
    return gulp
        .src(`${src}/js/*.js`) // change to your source directory
        .pipe(terser())
...
const imgopt = () => {
    return gulp
        .src(`${src}/assets/img/*.{jpg,png}`) // change to your source directory
        .pipe(
            imagemin([
...
const watch = () =>
    gulp.watch(
        ...
        gulp.series(css, html, js, imgopt, webp, assets, reload)
    );
...
const dev = gulp.task(
    "dev",
    gulp.series(
        gulp.parallel(css, html, js, imgopt, webp, assets, reload),
        serve,
        watch
    )
);
...
const build = gulp.task(
    "build",
    gulp.parallel(css, html, js, imgopt, webp, assets)
);
